<template>
  <div ref="dateColRef" class="date-col border-1" :class="{
    ...
    <!-- monthly -->
    <p v-if="!isWeekly && (taskPending > 0 || isToday)" class="task-mark" :class="{
      'mark-past': isPast,
      'mark-today': isToday,
      'mark-future': isFuture,
    }">
      {{ isToday ? taskDone + "/" + taskTotal : taskPending }}
    </p>
...
const isSameMonth = computed<boolean>(
  () => refSelected.value.getMonth() == refCalender.value.getMonth(),
);

const isSelected = computed<boolean>(() =>
  isSameDay(refSelected.value, refCalender.value),
);
